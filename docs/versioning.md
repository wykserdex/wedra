...
|---|---|---|---|
| Product / application | `VERSION` | `0.31b` | текущая рабочая линия после `v0.31a`; финальный релиз после аудита — `0.32` |
...
используется SemVer prerelease (`X.Y.Z-dev`). Стандартный стабильный релиз
публикуется как `X.Y.Z` с тегом `vX.Y.Z`; для отдельно одобренных
исправок допускается letter-suffix (`v0.31b`).
...
опубликованной версии плюс один minor. Буквенный suffix допускается только
как явно одобренный отдельный релиз; старые `v9`, `v10` и suffixes ниже
`v0.29` считаются историческими обозначениями и не выбирают номер следующего
релиза.

Release workflow отклоняет tag, если он не совпадает с `VERSION`. Перед
релизом `VERSION` меняется с development-версии на выбранную release-версию.
