...
## Если снапшот потерялся

Курсор resume берётся из журнала (`item_end`), а состояние — из `context.json`.
Если снимок не записался (контекст больше 16 МБ, диск/права), в журнале
появляется событие `snapshot_lost` с `reason` и `bytes`, ран завершается
терминальным `run_failed` с кодом `snapshot_lost` (не `run_end`), и
`--resume` такой ран наследует: продолжать можно, но «успешный» вердикт был бы
ложью — на диске лежит состояние более раннего элемента.
