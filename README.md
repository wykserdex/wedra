...
> M1–M5 закрыты, M6 GUI в работе (срезы 1–3: консоль, браузерный гейт, редактор). Честная версия: **v0.26a**.
...
- **Честный скоуп**: `when`, `foreach`, `parallel_group`, `retry`,
  `secrets`, `network` и type-объявления в input редактор не управляет —
  такие пайплайны открываются с баннером, а сохранение из редактора
  запрещено (409) — данные не теряются, правь в YAML во вкладке «Пайплайны».
- Сохранение: `PUT /api/pipelines/<file>` (существующий механизм, валидация
  до записи), новый файл создаётся.

Тесты: +4 (parse single_check: шаги/гейт/input; unsupported: when/
parallel_group/type-input → 409 на serialize; round-trip: parse→doc→serialize
→ YAML читается ядром и валиден, pos возвращается; новый гейт-пайплайн с нуля).
CI: live-шаг «v0.25 editor» (node --check + parse трёх examples, включая
when_demo → unsupported).
