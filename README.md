...
# WEDRA v0.28 — управляющий поток в редакторе: `when`, `foreach`, `parallel`, `after_foreach`
...
## Что нового в v0.28 (foreach / parallel / after_foreach в редакторе)
...
- **Честный скоуп**: `when` (v0.27) и `foreach`/`parallel_group`/
  `after_foreach` (v0.28) под управлением; `retry`, `secrets`, `network` и
...
Тесты: +10 (v0.27: when parse/round-trip/коэрсия; v0.28: parse трёх
foreach/parallel-примеров, pipeline-foreach round-trip, step-foreach на
фикстурном плагине, конфликты → ok:false).
