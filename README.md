...
# WEDRA v0.27 — `when`-узел в редакторе: условие шага под управлением, а не только в YAML
...
> M1–M5 закрыты, M6 GUI в работе (срезы 1–3: консоль, браузерный гейт, редактор). Честная версия: **v0.27**.
...
## Что нового в v0.27 (when в редакторе)
...
- **Осталось в YAML вручную**: `foreach`, `parallel_group`, `after_foreach`,
  `retry`, `secrets`, `network`, type-объявления input (следующие срезы).
...
- **Честный скоуп**: `when` под управлением с v0.27; `foreach`,
  `parallel_group`, `after_foreach`, `retry`, `secrets`, `network` и
  type-объявления в input редактор не управляет — такие пайплайны открываются
  с баннером, а сохранение из редактора запрещено (409) — данные не теряются,
  правь в YAML во вкладке «Пайплайны».
- Сохранение: `PUT /api/pipelines/<file>` (существующий механизм, валидация
  до записи), новый файл создаётся.

Тесты: +6 (v0.27: parse when_demo → when в doc, unsupported пуст; round-trip
when на гейт-синтетике; truthy без value + коэрсия «5»→5 для gt).
CI: live-шаг «v0.25 editor» (node --check + parse трёх examples; v0.27:
when-структура when_demo, serialize ok, коэрсия value, бад-op → ok:false).
