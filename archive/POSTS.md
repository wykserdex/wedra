...
> **Устарело (v9-цифры: 93 теста, 7 плагинов, «нет resume», «нет foreach steps.*»).**
> Публиковать как есть нельзя: цифры взять из README/CHANGELOG, «нет resume/foreach»
> — уже реализовано, релизы строятся автоматически на тег (не zip из воркспейса).
> Публичная волна отложена в пользу персональной волны 2 — см. `OUTREACH_ROUND2.md`.
> Чек-лист: п.1 (токен) ещё актуален; п.2 устарел (auto-release).
