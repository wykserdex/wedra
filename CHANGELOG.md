...
- **Литерал в `bind` больше не игнорируется молча.** `top_n: 1` не является
  ссылкой на контекст и не резолвился никогда: для optional-порта движок его
  пропускал, плагин брал значение по умолчанию, валидация проходила, ран
  завершался `done` — пайплайн тихо делал не то, что задано в YAML. Новый код
  `E_BIND_SOURCE_INVALID` (ошибка) + защита в рантайме: объявленный bind, который
  не резолвится, больше не считается «optional-порт не передан». Найдено прогоном
  настоящего пайплайна через MCP.
- Валидатор предупреждает о пути хоста в bind: абсолютный путь или выход за
  каталог (`..`) у плагина с `filesystem` не `readwrite` — `W_FILESYSTEM_HOST_PATH`.
- MCP: отказ политики (`network_denied`, `plugin_outside_root`,
  `file_ref_outside_root`) в `validate_pipeline` возвращается как
  `{ok:false, issues:[...]}` — по контракту инструмента это нормальный результат
  проверки, а не JSON-RPC ошибка. `run_pipeline` по-прежнему отказывает,
  fail-closed не ослаблен; тесты проверяют обе половины.
- MCP: `run_pipeline` больше не отдаёт `status: "done"` для провалившегосого рана
  (прерванные шаги = провал, семантика как в CLI).
- MCP: `get_run`/`runStatus` читают статус из журнала и для рана, запущенного
  другим процессом, вместо раннего `unknown`.
- Находки 3–4 получены прогоном настоящих пайплайнов через MCP
  (`text_analyzer`, `word_freq`, `email_extractor`, `foreach`); на каждую
  добавлены регрессионные тесты.

## 0.32b — sandbox scratch fixes

- Исправлен баг bwrap-песочницы: приватный scratch монтировался на `/tmp`, из-за
  чего каталог плагина, установленного во временный каталог, становился
  невидимым внутри песочницы и процесс не стартовал. Теперь ядро создаёт
  приватный scratch-каталог на каждый запуск и монтирует его на запись, а `/tmp`
  и каталог плагина остаются read-only.
- Приватный scratch-каталог создаётся на каждый запуск и удаляется после него;
  в нём направлены `HOME`/`TMPDIR` обоих бэкендов. Раньше на macOS плагин
  получал read-only `TMPDIR` хоста и не мог использовать свой scratch.
...
- CI: bwrap-песочница проверяется поведенчески в контейнере с privileged user
  namespaces (на GitHub-раннерах userns запрещены, поэтому штатный Linux-джоб
  покрывал только fail-closed ветку и форму аргументов).
...
- Единая схема версий: продукт живёт в `VERSION` и следует SemVer; протокол —
  в `protocol/VERSION` и не меняет продуктовую версию.
- Текущая ветка продолжает историю после опубликованных `v0.29`; старые
  обозначения ниже сохранены только как исторические записи.
- Run IDs получили случайный suffix, а новый журнал создаётся с `O_EXCL`:
  параллельные запуски больше не перезаписывают друг друга.
- GUI polling кэширует summary по mtime; JSON store индексирует runs/artifacts,
...
- `wedra approve` — TTY-подтверждение намерения и подсказка защищённого API-вызова; окончательное решение отправляет человек из GUI/API с сессией.
...
- Палитра→холст: нативный HTML5 DnD (draggable/dragstart/drop) заменён на
  mouse-события: mousedown → ghost-элемент следует за курсором →
  mouseup над холстом = добавить узел; клик без движения = добавить на
  свободное место. Причина: нативный DnD не работает в sandboxed iframe
  (превью: sandbox="allow-scripts" без allow-same-origin — Chromium
  гасит drag-сессию); mouse-события работают везде (превью, WebView2,
  браузер). Движение узлов (мышь) и связи (select) не тронуты.
- Подсветка зоны дропа (drop-over), ghost-подсказка, hint обновлён.
