...
- Валидатор предупреждает о пути хоста в bind: абсолютный путь или выход за
  каталог (`..`) у плагина с `filesystem` не `readwrite` — новый код
  `W_FILESYSTEM_HOST_PATH`. Раньше это выяснялось только в рантайме
  (`path_escape`), хотя валидатор знает и значение, и манифест плагина.
...
- MCP: `run_pipeline` больше не отдаёт `status: "done"` для провалившегося рана.
  Прерванные шаги считаются провалом (семантика как в CLI, `runExitCode`): раньше
  агент получал «done» для рана, где все шаги упали, потому что статус выводился
  только из ошибки, а у такого рана она nil.
- MCP: `get_run`/`runStatus` читают статус из журнала и для рана, запущенного
  другим процессом. Раньше был ранний `return "unknown"`, из-за которого
  `get_run` отдавал `unknown` вместе с полным списком событий; несуществующий
  `run_id` по-прежнему даёт `unknown`, а не `running`.
- Все четыре находки получены прогоном настоящего пайплайна через MCP, а не
  тестом; на каждую добавлены регрессионные тесты.
